...
mod ffi;
#[cfg(target_os = "macos")]
use ffi::{nodox_macos_apply_window_liquid_glass, nodox_macos_greet};
#[cfg(target_os = "macos")]
...
pub fn apply_window_liquid_glass(window_ptr: *mut c_void) -> bool {
    #[cfg(target_os = "macos")]
    {
        ...
        return unsafe { nodox_macos_apply_window_liquid_glass(window_ptr as Int) };
    }
...
pub fn greet(name: &str) -> Option<String> {
    #[cfg(target_os = "macos")]
    {
        ...
        return unsafe { nodox_macos_greet(&swift_name) }.map(|value| value.as_str().to_string());
    }
